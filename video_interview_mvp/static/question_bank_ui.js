(() => {
  ...
  root.innerHTML = `
    <div class="card p-5">
      <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div><h2 class="font-bold">Банк технических вопросов</h2><p class="text-sm text-gray-500 mt-1">Группировка по компетенциям, поиск по тексту и тегам. Possible extra questions — подсказки для адаптивных уточнений, а не обязательный сценарий.</p></div>
      ...
      <div class="grid md:grid-cols-[1fr_220px_220px] gap-3 mt-5">
        <input id="qbSearch" class="px-4 py-3 border rounded-xl" placeholder="Поиск по вопросу, критериям, доп. вопросам или тегам">
  ...
  modal.innerHTML = `
    <div class="bg-white rounded-3xl w-full max-w-3xl max-h-[94vh] overflow-y-auto soft">
      ...
      <div class="p-6 space-y-4">
        ...
        <div class="p-4 bg-indigo-50 rounded-2xl"><label class="text-sm font-semibold text-indigo-900">Possible extra questions</label><p class="text-xs text-indigo-700 mt-1">Необязательные подсказки LLM для уточняющих вопросов. По одному вопросу на строку.</p><textarea id="qbExtra" rows="5" class="w-full mt-3 px-4 py-3 border rounded-xl bg-white" placeholder="Как бы изменилось решение при...?\nПочему вы выбрали именно этот подход?"></textarea></div>
      </div>
  ...
  function render() {
    ...
    const filtered = items.filter(item => {
      const haystack = [item.question, item.competency, ...(item.tags || []), item.reference_answer, ...(item.must_have || []), ...(item.nice_to_have || []), ...(item.red_flags || []), ...(item.possible_extra_questions || [])].join(' ').toLowerCase();
    ...
    document.getElementById('questionBank').innerHTML = [...groups.entries()].map(([name, questions]) => `
      ...
      <div class="grid lg:grid-cols-2 gap-4">${questions.map(item => `
        ...
        <div class="text-xs text-indigo-600 mt-2">${(item.possible_extra_questions || []).length} подсказок для уточнений</div>
        <details class="mt-3"><summary class="text-xs font-semibold text-gray-500 cursor-pointer">Критерии, референс и уточнения</summary><div class="mt-3 text-xs text-gray-600 space-y-3"><div><b>Must have:</b><ul class="list-disc pl-5 mt-1">${(item.must_have || []).map(x => `<li>${esc(x)}</li>`).join('') || '<li>—</li>'}</ul></div><div><b>Nice to have:</b><ul class="list-disc pl-5 mt-1">${(item.nice_to_have || []).map(x => `<li>${esc(x)}</li>`).join('') || '<li>—</li>'}</ul></div><div><b>Red flags:</b><ul class="list-disc pl-5 mt-1">${(item.red_flags || []).map(x => `<li>${esc(x)}</li>`).join('') || '<li>—</li>'}</ul></div><div><b>Possible extra questions:</b><ul class="list-disc pl-5 mt-1">${(item.possible_extra_questions || []).map(x => `<li>${esc(x)}</li>`).join('') || '<li>—</li>'}</ul></div><div><b>Reference:</b><div class="mt-1 leading-5">${esc(item.reference_answer || '—')}</div></div></div></details>
  ...
  function openEdit(id = null) {
    ...
    document.getElementById('qbExtra').value = (item?.possible_extra_questions || []).join('\n');
  ...
  async function save() {
    const payload = {
      ...
      possible_extra_questions: splitLines(document.getElementById('qbExtra').value)
    };
